...
so the cases below sit exactly on 0x20, 0x2F, 0x30, 0x80, 0xF8, on an SFR
address that is not a multiple of eight, on the first bit past the top of
the RAM window, and on the first offset past the eighth bit of a byte.
...
ASM_CASES = [
    ...
    # An offset is a bit of the byte the operand names, and a byte has
    # eight of them.  B2B(0x20,8) is bit 0 of the byte at 0x21, which is
    # not what was written, and B2B(0xf8,8) is off the end of the bit
    # space; both are refused wherever the fold is made.
    acase('b2b-ram-past-bit7', 'setb B2B(0x20,8)', None, NOTRANGE),
    acase('b2b-sfr-past-bit7', 'setb B2B(0xf8,8)', None, NOTRANGE),
    # Only every eighth byte of the SFR page is bit addressable.  This arm
    # of the constant fold used to be reached by every byte from 0x80 up,
    # so B2B(0x84,1) folded to bit address 0x85, which is P0.5 and has
    # nothing to do with the byte at 0x84, while the same operand through
    # md_apply_fix or through the linker was refused.
    acase('b2b-sfr-unalign', 'setb B2B(0x84,1)', None, NOTRANGE),
    acase('b2b-sfr-top',     'setb B2B(0xff,0)', None, NOTRANGE),
    ...
    acase('b2b-sym-unalign', 'BD = 0x84\n\tsetb B2B(BD,1)',  None, NOTRANGE),
    ...
    acase('b2b-fwd-unalign', 'setb B2B(BD,1)\nBD = 0x84',     None, NOTBIT),
    acase('b2b-fwd-gap',     'setb B2B(BD,0)\nBD = 0x40',     None, NOTBIT),
    acase('b2b-fwd-past-top', 'setb B2B(BD,9)\nBD = 0x2f',    None, NOTRANGE),
    acase('b2b-fwd-ram-past-bit7', 'setb B2B(BD,8)\nBD = 0x20', None, NOTRANGE),
    acase('b2b-fwd-sfr-past-bit7', 'setb B2B(BD,8)\nBD = 0xf8', None, NOTRANGE),
]
...
LINK_CASES = [
    ...
    lcase('link-ram-past-bit7', 0x20, 8, None, 'relocation truncated'),
    lcase('link-sfr-past-bit7', 0xf8, 8, None, 'relocation truncated'),
