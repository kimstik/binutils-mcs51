...
Assembly cases are assembled alone and their bytes compared with the
expected encoding; a case without expected bytes must make the assembler
exit nonzero and say why.  The bytes are read from the section the case
names, .text unless it says otherwise, because a directive emits into the
memory space it was given.  Link cases assemble one B2B against an
undefined symbol and link it with the byte address supplied by --defsym,
so the fold being checked is the linker's.
"""
...
class Tools:
    ...
    def section(self, elf, work, name):
        """The bytes of one named section, or None if there is no such
        section - which is how a directive that emitted nothing reads."""
        b = os.path.join(work, 'sec.bin')
        if os.path.exists(b):
            os.remove(b)
        r = subprocess.run([self.objcopy, '--dump-section',
                            '%s=%s' % (name, b), elf,
                            os.path.join(work, 'dump.elf')],
                           capture_output=True, text=True)
        if r.returncode or not os.path.exists(b):
            return None
        return open(b, 'rb').read()
...
def main():
    ...
    with tempfile.TemporaryDirectory() as work:
        for c in bits.ASM_CASES + bits.DIR_CASES:
            ...
            flat = ' ; '.join(' '.join(l.split())
                              for l in c['src'].splitlines())
            if c['want'] is None:
                if obj is not None:
            ...
            if obj is None:
                ...
                continue
            sec = c.get('section', '.text')
            got = (t.text(obj, work) if sec == '.text'
                   else t.section(obj, work, sec))
            if got is None:
                bad.append('%-18s %-30s no %s in the object'
                           % (c['name'], flat, sec))
                continue
            want = bytes.fromhex(c['want'])
            if got != want:
                bad.append('%-18s %-30s %s: want %s got %s'
                           % (c['name'], flat, sec, want.hex(),
                              got.hex() or 'nothing'))
