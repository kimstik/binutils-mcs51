...
# the space it belongs to.  Two further things are checked here:
#
...
"$READELF" -s --wide "$W/commons.o" | tr -d '\r' > "$W/syms.txt"

ndx() { awk -v s="$1" '$NF == s { print $(NF-1); exit }' "$W/syms.txt"; }
expect_ndx() {
    v=$(ndx "$1")
    [ -n "$v" ] || { fail "symbol $1 not in the object"; return; }
    [ "$v" = "$2" ] || fail "$1 has Ndx $v, expected $2"
}
expect_ndx RCVAR  'PRC[0xff01]'
expect_ndx BCVAR  'PRC[0xff02]'
expect_ndx ICVAR  'PRC[0xff03]'
expect_ndx XCVAR  'PRC[0xff04]'
expect_ndx ECVAR  'PRC[0xff05]'
expect_ndx BITVAR 'PRC[0xff06]'
expect_ndx CVAR   COM
...
"$READELF" -S --wide "$W/commons.elf" | tr -d '\r' \
    | sed 's/^ *\[ *[0-9]*\] *//' > "$W/secs.txt"

expect_size() {
    line=$(awk -v s="$1" '$1 == s { print; exit }' "$W/secs.txt")
    [ -n "$line" ] || { fail "section $1 not in the linked image"; return; }
    sz=$(echo "$line" | awk '{ print $5 }')
    [ $((0x$sz)) -eq $(($2)) ] || fail "$1 is 0x$sz bytes, expected $2"
}
expect_size .rbss   4
expect_size .bbss   2
expect_size .ibss   8
...
echo "run-commons: PASS (commons and space directives keep their space, their name, and the real sections beside them)"
