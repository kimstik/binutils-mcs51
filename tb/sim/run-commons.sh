...
#   - one name declared a common of two different spaces is refused.
#     The spaces are disjoint address spaces and the linker's own
#     arbitration knows nothing of them: it keeps the larger of the two
#     and, at equal sizes, the one it saw first, so link order alone used
#     to decide the answer.  The same name in the SAME space in two
#     objects, and a plain .comm beside a space common, must still link.
#
...
#
# exit: 0 pass, 1 a common lost its space, a common or a directive ate a
#       real section, or one name in two spaces was not refused (or an
#       ordinary repeated common was), 2 bad usage/toolchain
...
# ---- one name in two memory spaces ---------------------------------
# The spaces are disjoint address spaces of the machine, and the
# arbitration between two commons of one name knows nothing of them: it
# keeps the larger of the two and, at equal sizes, the one it saw first.
# `.xcomm CONFVAR,4' in one object and `.icomm CONFVAR,4' in another
# therefore linked with nothing said, and the order of the two files on
# the command line alone decided whether CONFVAR was external RAM at
# 0x0000 or internal RAM at 0x0020.  No address satisfies both
# declarations, so the link is refused.  The diagnostic has to name the
# symbol, both spaces and both objects: that is the whole of what a
# reader needs to reach the two declarations.
cat > "$W/confx.s" <<'EOF'
; CONFVAR in the external RAM space
...
EOF
cat > "$W/confi.s" <<'EOF'
; the same name in the indirectly addressed internal RAM space

        .icomm CONFVAR,4,1
EOF

"$AS" -o "$W/confx.o" "$W/confx.s" \
    || { echo "run-commons: as-new failed on confx.s" >&2; exit 1; }
"$AS" -o "$W/confi.o" "$W/confi.s" \
    || { echo "run-commons: as-new failed on confi.s" >&2; exit 1; }

expect_says() {  # WHAT ERR-FILE WORD..., each word somewhere in the message
    what=$1
    err=$2
    shift 2
    for w in "$@"; do
        grep -qF -- "$w" "$err" \
            || fail "$what: the diagnostic does not name $w"
    done
}

# Both orders, because the order is exactly what used to decide the
# answer, and a check that fires only one way round leaves half the bug.
if "$LD" -o "$W/conflict.elf" "$W/confx.o" "$W/confi.o" 2> "$W/conflict.err"; then
    fail "conflict: the link succeeded - CONFVAR is .xcomm in confx.o and .icomm in confi.o"
else
    expect_says conflict "$W/conflict.err" CONFVAR .xbss .ibss confx.o confi.o
fi

if "$LD" -o "$W/conflictr.elf" "$W/confi.o" "$W/confx.o" 2> "$W/conflictr.err"; then
    fail "conflict reversed: the link succeeded - CONFVAR is .icomm in confi.o and .xcomm in confx.o"
else
    expect_says "conflict reversed" "$W/conflictr.err" CONFVAR .xbss .ibss confx.o confi.o
fi

# A relocatable link merges two commons the same way a final one does,
# so it has to refuse the same pair; letting it through only carries the
# wrong answer into the merged object, where the space is gone.
if "$LD" -r -o "$W/conflict.o" "$W/confx.o" "$W/confi.o" 2> "$W/conflictp.err"; then
    fail "conflict under ld -r: the relocatable link succeeded"
else
    expect_says "conflict under ld -r" "$W/conflictp.err" CONFVAR .xbss .ibss
fi

# The control that matters: one name in the SAME space in two objects is
# the ordinary case and has to link exactly as it does today.  Without
# it the check above could be refusing every repeated common and nothing
# here would notice.
cat > "$W/samea.s" <<'EOF'
; SVAR in the external RAM space
...
EOF
cat > "$W/sameb.s" <<'EOF'
; the same name in the same space

        .xcomm SVAR,4,1
EOF

"$AS" -o "$W/samea.o" "$W/samea.s" \
    || { echo "run-commons: as-new failed on samea.s" >&2; exit 1; }
"$AS" -o "$W/sameb.o" "$W/sameb.s" \
    || { echo "run-commons: as-new failed on sameb.s" >&2; exit 1; }

expect_same() {  # WHAT OUT-ELF OBJ OBJ
    if ! "$LD" -o "$2" "$3" "$4" 2> "$W/same.err"; then
        fail "$1: linking two commons of one space FAILED"
        cat "$W/same.err"
        return
    fi
    if [ -s "$W/same.err" ]; then
        echo "== ld stderr ($1, non-fatal):"; cat "$W/same.err"
    fi
    sz=$(secfield "$2" .xbss 5)
    if [ -z "$sz" ]; then
        fail "$1: .xbss not in the image"
        return
    fi
    # One common, not two: a check that refused the pair outright would
    # leave the space empty, and one that let both through would double it.
    [ $((0x$sz)) -eq 4 ] || fail "$1: .xbss is 0x$sz bytes, expected 4"
    # Where .xbss starts is the script's arithmetic; that SVAR is the thing
    # at the start of it is what this arm is about.
    at=$(secfield "$2" .xbss 3)
    v=$(symval "$2" SVAR)
    if [ -z "$v" ]; then
        fail "$1: symbol SVAR not in the image"
    elif [ $((0x$v)) -ne $((0x$at)) ]; then
        fail "$1: SVAR = 0x$v, expected 0x$at, the start of .xbss"
    fi
}
expect_same "same space"          "$W/same.elf"  "$W/samea.o" "$W/sameb.o"
expect_same "same space reversed" "$W/same2.elf" "$W/sameb.o" "$W/samea.o"

# A plain .comm names no space, and one name declared .comm in one
# object and .icomm in another still links.  That is not a nicety: the
# objects this port is used with carry two builds of one library side by
# side, the same names declared with .comm before the space directives
# existed and with .bcomm or .icomm after, and 33 names in tb/base.7z
# are declared both ways.  Which space such a pair lands in is decided
# by link order to this day - a separate defect, and nothing here pins
# it.  What is pinned is that the link is still allowed.
cat > "$W/plaina.s" <<'EOF'
; PVAR as an ordinary common
...
EOF
cat > "$W/plainb.s" <<'EOF'
; the same name as a memory-space common

        .icomm PVAR,4,1
EOF

"$AS" -o "$W/plaina.o" "$W/plaina.s" \
    || { echo "run-commons: as-new failed on plaina.s" >&2; exit 1; }
"$AS" -o "$W/plainb.o" "$W/plainb.s" \
    || { echo "run-commons: as-new failed on plainb.s" >&2; exit 1; }

expect_plain() {  # WHAT OUT-ELF OBJ OBJ
    if ! "$LD" -o "$2" "$3" "$4" 2> "$W/plain.err"; then
        fail "$1: linking a .comm and an .icomm of one name FAILED - the space check reaches ordinary commons"
        cat "$W/plain.err"
        return
    fi
    if [ -s "$W/plain.err" ]; then
        echo "== ld stderr ($1, non-fatal):"; cat "$W/plain.err"
    fi
    v=$(symval "$2" PVAR)
    [ -n "$v" ] || fail "$1: symbol PVAR not in the image"
}
expect_plain "plain common"          "$W/plain.elf"  "$W/plaina.o" "$W/plainb.o"
expect_plain "plain common reversed" "$W/plain2.elf" "$W/plainb.o" "$W/plaina.o"
...
if [ "$st" -ne 0 ]; then
    ...
    echo "== ld on one name in two spaces";          cat "$W/conflict.err"
    echo "== ld on the same two files reversed";     cat "$W/conflictr.err"
    echo "== ld -r on one name in two spaces";       cat "$W/conflictp.err"
    exit 1
fi
echo "run-commons: PASS (commons keep their space through a relocatable link, one name in two spaces is refused and named, space directives keep their name, and the real sections beside them keep their bytes)"
