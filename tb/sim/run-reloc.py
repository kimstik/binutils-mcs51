...
does, in four steps:
  ...
  fold      the same relocations when gas resolves them itself, because the
            symbol turns out to be local and absolute: the second copy of
            every range rule lives there, in md_apply_fix, and HIGH()/LOW()
            reach nothing else at all.
...
PCODE_LINK = [
    ...
    ('pcode-link-wrap',  0x2345, '034500', None),   # above 13 bits: masked
...
def main():
    ...
    with tempfile.TemporaryDirectory() as work:
        ...
        for name, value, want, msg in PCODE_LINK:
            ...
            if want is None:
                if elf is None or msg not in log:
