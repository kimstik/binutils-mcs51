...
would test binutils, not the port.  Comments are masked out before the line
operators match, so a `>' or a literal that occurs only in prose is not
mutated: such a mutant produces the same object as the clean tree, so no
stage can ever kill it and its only effect is to understate the kill rate.
...
before/after comparison is on exactly the same mutants.  An operator can
match one line twice - two comparisons in one condition, several literals in
one expression - so the id carries an ordinal in that case; ids are unique
because run.py --only and report.py both key on them.
...
def mask_comments(lines):
    """The same lines with every comment character replaced by a space.

    Column positions and line lengths are preserved, so an operator matches
    against the masked text and builds its mutant out of the original line at
    the same offsets: a line whose code half is real and whose tail is a
    comment is still mutated in its code half.  String and character literals
    are stepped over so a `/*' inside one does not open a comment; a quote
    with no closing quote on the line is ordinary text, which is what an
    apostrophe in a shell comment of the linker script template is."""
    out = []
    inblock = False
    for s in lines:
        r = list(s)
...
def _literal_end(s, i):
    """Index just past the literal opening at S[I], or I if it does not close
    on this line."""
    quote, j = s[i], i + 1
    while j < len(s):
        if s[j] == '\\':
...
# --------------------------------------------------------------- operators
#
# Every line operator takes (LINES, IDX, CODE): CODE is LINES with the
# comments masked out and is what the operator matches against, LINES is what
# the mutant is built from.  The two are aligned character for character, so
# an offset found in one indexes the other.

def op_relop(lines, idx, code):
    swap = {'<': '<=', '<=': '<', '>': '>=', '>=': '>'}
    for i in idx:
        s, c = lines[i], code[i]
        if not re.search(r'0x[0-9A-Fa-f]|[^\w.]\d', c):
            continue
...
def op_constpm1(lines, idx, code):
    for i in idx:
        s, c = lines[i], code[i]
        # Preprocessor lines only: a comment carries no literal to find once
        # it is masked.
        if c.lstrip().startswith('#'):
            continue
        for m in re.finditer(r'\b(0[xX][0-9A-Fa-f]+|[0-9]+)\b', c):
            tok = m.group(1)
...
def op_guard0(lines, idx, code):
    ...
    for i in idx:
        ...
        body = ' '.join(code[i:min(i + 5, len(code))])
...
def op_cond0(lines, idx, code):
    ...
    for i in idx:
        ...
        if not m:
            continue
        depth, end = 0, None
        for col in range(m.end() - 1, len(c)):
            if c[col] == '(':
                depth += 1
            elif c[col] == ')':
                depth -= 1
                if depth == 0:
                    ...
                    break
        if end is None or c[m.end() - 1:end + 1] == '(0)':
            continue
        yield (i, s[:m.end()] + '0' + s[end:], 'condition forced to 0')


def op_retstatus(lines, idx, code):
    for i in idx:
        s, c = lines[i], code[i]
        for what in ('bfd_reloc_overflow', 'bfd_reloc_outofrange'):
            ...
            k = c.find(old)
            if k >= 0:
                yield (i, s[:k] + 'return bfd_reloc_ok' + s[k + len(old):],
                       '%s -> ok' % what)
...
def op_endian(lines, idx, code):
    for i in idx:
        s, c = lines[i], code[i]
        for a, b in ENDIAN:
            k = c.find(a)
            if k >= 0:
                yield (i, s[:k] + b + s[k + len(a):], '%s -> %s' % (a, b))


def op_oporder(lines, idx, code):
    inset = set(idx)
    for i in idx:
        if i + 1 not in inset:
            continue
        # The pair is judged on the code halves; the swap moves the whole
        # source lines, comments included.
        a, b = code[i].strip(), code[i + 1].strip()
        ...
        yield (i, [ind_a + lines[i + 1].strip(), ind_b + lines[i].strip()],
               'adjacent operand statements swapped')


def op_boolarg(lines, idx, code):
    for i in idx:
        s, c = lines[i], code[i]
        for m in re.finditer(r'(?<![\w>])(true|false)(?=\s*,)', c):
            if 'oper' not in c and 'exp' not in c:
                continue
...
def op_howto(lines, _idx, _code):
    # Matches the unmasked text on purpose: the field this perturbs is named
    # by the `/* size */' comment beside it, so the mask would hide it.
    cur = None
...
def op_ldnum(lines, _idx, code):
    body = False
    for i, s in enumerate(lines):
        c = code[i]
        if c.startswith('MEMORY') or c.startswith('SECTIONS'):
            body = True
        # `#' is the shell comment of the template around the heredoc; the
        # script's own /* */ comments are already masked out of C.
        if not body or c.lstrip().startswith('#'):
            continue
        for m in re.finditer(r'\b(0[xX][0-9A-Fa-f]+|[0-9]+)\b', c):
            tok = m.group(1)
            if tok.lower().startswith('0x'):
...
# opcode table has 111 rows and the howto table 14 entries with six mutable
...
def main():
    ...
    for key, rel in FILES.items():
        ...
        lines = read(path)
        code = mask_comments(lines)
        idx = scoped_lines(key, lines)
        for name, fn, keys in OPERATORS:
            if key not in keys or (only and name not in only):
                continue
            # One operator can match a line more than once - two comparisons
            # in one condition, several literals in one expression.  Number
            # each match within its line before the cap samples, so the
            # ordinal names the same match whatever --cap is, and so the
            # second match is a mutant of its own rather than a duplicate id
            # of the first.  A line matched once keeps the bare
            # file-operator-line id.
            nth = collections.Counter()
            ranked = []
            for line, new, note in fn(lines, idx, code):
                nth[line] += 1
                ranked.append((line, new, note, nth[line]))
            for line, new, note, k in spread(ranked, CAPS.get(name, args.cap)):
                old = lines[line]
                rep = new if isinstance(new, list) else [new]
                orig = lines[line:line + len(rep)]
                if orig == rep:
                    continue
                mid = '%s-%s-%d' % (key, name, line + 1)
                if nth[line] > 1:
                    mid += '.%d' % k
                mutants.append({
                    'id': mid,
    ...
    # Ids are unique by construction above.  This says so out loud rather
    # than dropping the second mutant of a colliding pair, which is a hole in
    # the kill rate that nothing in the run would report: run.py --only and
    # report.py both address a mutant by its id.
    seen = set()
    for m in mutants:
        if m['id'] in seen:
            sys.exit('duplicate mutant id %s' % m['id'])
        seen.add(m['id'])

    text = json.dumps(mutants, indent=1)
    if args.out == '-':
        print(text)
    else:
        with open(args.out, 'w') as f:
            f.write(text + '\n')
        print('%d mutants -> %s' % (len(mutants), args.out), file=sys.stderr)
    return 0
